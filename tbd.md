...
兼容边界也必须明确：主题只能修复展示、轮询、查询和前端状态同步。Komari 服务端已经发出的错误离线/上线通知，主题无法取消或改写，不能用伪造前端在线状态来掩盖。
...
## 本轮完成：主题侧状态与刷新加固（2.0.28）

- [x] latest status 使用完整快照更新；快照缺失的节点不再永久保留旧的 `online=true`。
- [x] 节点列表和 latest status 改为独立结算；其中一个轮询失败时仍应用另一个成功结果。
- [x] 删除轮询中无业务价值的额外 `rpc.ping()`，避免第三个请求拖垮整轮更新。
- [x] 对轮询顶层响应增加非空对象校验，避免异常 `null` / 数组直接破坏 store。
- [x] 负载请求增加 generation，旧节点、旧区间和旧定时请求不能覆盖新结果。
- [x] 负载实时定时器现在明确启动，并且只在实时模式按主题间隔后台更新。
- [x] 历史负载改为每分钟无闪烁后台刷新，失败时保留现有图表。
- [x] 旧负载 REST 回退复用共享 API 客户端，统一 base URL、Cookie、超时、解包和错误语义。
- [x] 严格区分 definitions 的未加载、明确不支持和新版数组三态；非 MethodNotFound 错误不回退旧接口。
- [x] 指标 definitions 和 query/stats 能力缓存增加 60 秒 TTL，并用 generation 防止 reset 后旧请求回写。
- [x] 已挂载的负载、Ping 图会定期重新探测 retention 和指标能力，无需整页硬刷新。
- [x] Ping 请求固定捕获 UUID 与时间窗口，异步期间切换节点不会混用新旧参数。
- [x] Ping 历史每分钟无闪烁后台刷新；后台失败保留旧图和任务。
- [x] Ping 混合周期按最快有效任务间隔选桶，不再因最慢任务把其他曲线压成单点。
- [x] 只有孤立有效点、没有可连线片段时显示 symbol，同时继续禁止跨 `null` 补线。
- [x] Ping 后台刷新保留用户显式“全不选”或自定义任务选择。
...
- [x] Ping 查询点数跟随任务上报周期；混合周期优先保留最快任务的连续曲线，较慢任务按真实孤立点显示。
...
- Komari 1.2.6/当前 Snapshot 的 v2 WebSocket report 仍会同时刷新 POST presence，而离线通知宽限计时按单个连接来源判断；WS/POST 来源切换时可能出现面板仍在线、服务端通知却反复离线/上线。主题只能准确展示 latest status，无法修复服务端通知器。
...
- [x] 本轮 `pnpm lint` 通过。
- [x] 本轮 `pnpm exec vue-tsc --noEmit -p tsconfig.app.json` 通过。
- [x] 本轮 `pnpm build` 通过，并按最终提交哈希生成 `komari-theme-naive-build-<sha>.zip`。
